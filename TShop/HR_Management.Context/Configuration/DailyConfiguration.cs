...
namespace HR_Management.Context.Configuration
{
    public class DailyConfiguration : EntityTypeConfiguration<Daily>
    {
        public DailyConfiguration()
        {
            ...
            Property(d => d.Invoice).HasColumnAnnotation("Index",
                                       new IndexAnnotation(new IndexAttribute("AK_DailyInvoice", 1) { IsUnique = true }));
            Property(ii => ii.HeadName).HasMaxLength(80).IsRequired()
        .HasColumnAnnotation("Index", new IndexAnnotation(new IndexAttribute("AK_DailyInvoice", 2) { IsUnique = true }));
        }
    }
}
