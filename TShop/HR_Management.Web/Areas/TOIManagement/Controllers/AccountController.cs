    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            try
            {
                ...
                Company company=  db.Companyies.FirstOrDefault();
                CompanyCookie.companyCookie(company.Id);
                var result = await SignInManager.PasswordSignInAsync(model.Email, model.Password, model.RememberMe, shouldLockout: false);
                switch (result)
                {
                    case SignInStatus.Success:
                        //  Log4NetHelper.Log(String.Format("Cannot Edit Employee {0} ", user.Id), LogLevel.INFO, "Login", user.Id, User.Identity.Name, null);
                        if (returnUrl != null)
                        {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                //  Guid companyGuid = new Guid("420e07db-0a63-40cc-ae4d-b0d8603ed864");
                // Company cmp= db.Companyies.FirstOrDefault();
        ...
        [HttpPost]
        [AllowAnonymous]
        public async Task<ActionResult> ReGenrateEmailConfirmation(string email)
        {
